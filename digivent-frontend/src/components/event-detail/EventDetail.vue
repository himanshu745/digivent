<template>
  <div>
    ...
    <div v-if="isSpeaker === 'yes'">
      <a href @click.prevent="checkSpeaker(event.speaker._id)">Edit event</a>
      <a href @click.prevent="deleteEvent(event._id, event.speaker._id)">Delete Event</a>
    ...
    <div v-else>
      <div class="flexbox">
        <router-link :to="{ name: 'speaker-detail', params: { speakerId: event.speaker._id } }">
          <h6>About host</h6>
        </router-link>
        <router-link :to="{ name: 'question', params: { eventId: event._id } }">View Questions</router-link>
      </div>
      <h4>Event description</h4>
      <p>{{ event.description }}</p>
      <div @click.prevent="bookEvent()">
        <router-link class="btn" :to="{ name: 'book', params: { event: event } }">Book</router-link>
      </div>
    </div>
  </div>
</template>
